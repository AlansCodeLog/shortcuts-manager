...
describe(testName(), () => {
	describe("base classes", () => {
		it("should correctly add/call/remove listeners", () => {
			...
			shortcut.set("chain", [[]])
			expect((listener as Mock<any, any>).mock.calls.length).to.equal(2)
	...
	describe("collection classes", () => {
		it("should correctly add/call/remove listeners", () => {
			...
			const listener: any = vi.fn(() => Ok(true))
			...
			expect((listener as Mock<any, any>).mock.calls.length).to.equal(1)
			shortcuts.add(new Shortcut([[]]))
			expect((listener as Mock<any, any>).mock.calls.length).to.equal(2)
			shortcuts.allows("remove", shortcuts.entries[0])
			expect((listener as Mock<any, any>).mock.calls.length).to.equal(3)
			shortcuts.remove(shortcuts.entries[0])
			expect((listener as Mock<any, any>).mock.calls.length).to.equal(4)
